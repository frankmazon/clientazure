...
export default function DocumentTypePage() {
  ...
  const clients = useMemo<Client[]>(() => {
    ...
    return data.filter((client: Client) => client.documentType === type);
  }, [type]);
  ...
  return (
    <DashboardLayout
      ...
      subtitle={`Client folders containing ${pageTitle} submissions`}
    >
      <div className="space-y-6">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">
            {clients.length} client folder(s) found
          </p>
        </div>
        ...
        <div className="grid gap-4">
          {clients.map((client) => {
            ...
            const isIdDocument = client.documentType === 'id';
